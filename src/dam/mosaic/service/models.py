            ...
            for y in np.arange(src.shape[1]):
                for x in np.arange(src.shape[0]):
                    ...
                    if adaptiveMethod == self.ADAPTIVE_THRESH_MEAN_C:
                        ...
                        threshold = (mask * cornerPixel[x: x + mask.shape[0],
                                            y: y + mask.shape[1]]).sum() / mask.sum() - C
...
class Canny(object):
    ...
    def get(self):
        ...
        weak = np.int32(25) #약한에지
        strong = np.int32(255) #강한에지

        #이중 임곗값 비교

        #최대 입곗값보다 큰 원소의 인덱스를 저장
...
def image_read(fname)->object:
    return(lambda x: cv2.imread('../../../static/data/dam/mosaic/+x'))(fname)
